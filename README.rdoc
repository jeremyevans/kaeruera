...
== Heroku Setup
  ...
  heroku config:set \
    KAERUERA_CIPHER_SECRET=`ruby -rsecurerandom -e 'puts SecureRandom.base64(24)'`
    KAERUERA_HMAC_SECRET=`ruby -rsecurerandom -e 'puts SecureRandom.base64(24)'`
...
First, create an .env.rb setting the KAERUERA_CIPHER_SECRET and
KAERUERA_HMAC_SECRET (session secrets, should be 32 bytes each)
and KAERUERA_DATABASE_URL (database connection URL) environment
variables.
