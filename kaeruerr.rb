...
require 'rack/csrf'
require 'models'
require './lib/kaeruerr'

class KaeruErrApp < Sinatra::Base
  KE = KaeruErr.new(DB.uri, 'jeremy', 'kaeruerr')

  set :environment, 'production'
  disable :run
  use Rack::Session::Cookie, :secret=>File.file?('kaeruerr.secret') ? File.read('kaeruerr.secret') : SecureRandom.hex(20)
